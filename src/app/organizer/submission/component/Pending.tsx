...
function Pending() { 
 ...
 const [filteredSubmissions, setFilteredSubmissions] = useState<{
  ...
  date: string
    ...
    const pendingSubmissions = Submission.filter(item => item.status === "Pending");
   ...
   {filteredSubmissions.map((sub, index) => (
      <tr key={index} className="hover:bg-gray-50 border-b border-[#EEEEEE]">
        ...
        <td className="px-4 py-5"> <div>
            ...
            <span className={`px-5 py-2 rounded-lg font-semibold bg-[#F9831C61] text-[#F9831C]  border-[#F9831C] border`}>
                {sub.status}
            </span>
        </td>
      </tr>
